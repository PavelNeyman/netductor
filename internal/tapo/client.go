...
// Control is a one-shot helper for agent/CLI: dir left|right|up|down|stop|night:|privacy:
func Control(host, user, password, dir string, step int) string {
	...
	case strings.HasPrefix(dir, "night:"):
		res, err = cl.SetDayNight(strings.TrimPrefix(dir, "night:"))
	case strings.HasPrefix(dir, "privacy:"):
		on := strings.TrimPrefix(dir, "privacy:")
		res, err = cl.SetPrivacy(on == "on" || on == "1" || on == "true")
	case dir == "left":
		res, err = cl.MoveMotor(-step, 0)
	case dir == "right":
		res, err = cl.MoveMotor(step, 0)
	case dir == "up":
		res, err = cl.MoveMotor(0, step)
	case dir == "down":
		res, err = cl.MoveMotor(0, -step)
	case dir == "stop":
		return "tapo-go:stop:noop"
	default:
