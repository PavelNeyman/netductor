# Residual risks (detailed) — v0.8.22
...
**Mitigation (0.8.22+):** Default listen **`127.0.0.1:80`**. Public HTTP only with explicit `-listen :80`. Prefer VPN path or HTTPS (`-https-listen` + certs) when domain exists.
