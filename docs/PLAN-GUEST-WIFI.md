...
Status: **v1 agent implementation (0.8.27)** — nft + captive redirect + VPN bypass; hardware e2e pending · not all sites need guest · opt-in in edge wizard / template.
...
| **G1** | UCI guest + isolation + VPN bypass | **done 0.8.27** |
| **G2** | MAC allow-list + TTL + nft sync | **done 0.8.27** |
| **G3** | Desk UI + join QR text | **done 0.8.23** |
| **G4** | Captive page + HTTP DNAT redirect | **done 0.8.27** |
| **G5** | TG/Admin remote grant + wait result | **done 0.8.27** (band_split rejected) |
