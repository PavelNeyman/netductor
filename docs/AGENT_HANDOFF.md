...
- **0.8.30:** SA4000 doctor fix; dead code sweep; API status constants
 (0.8.17–0.8.30)
...
wget -qO /usr/local/bin/netductor \
  https://github.com/PavelNeyman/netductor/releases/download/v0.8.30/netductor-linux-amd64
...
- Git: delete + `go-test` sample pipeline + post-receive auto-pipeline env
- Admin/TG delete; docs SELFHOST-GIT + OPEN_ITEMS synced to thin model (forge not default)
...
## Current baseline — v0.8.32 (2026-09-20)

### Release
- Tag: **v0.8.32** · https://github.com/PavelNeyman/netductor/releases/tag/v0.8.30
...
## Security review snapshot (v0.8.30)
...
4. **Hardcoded old download URLs** in some legacy docs/scripts (secondary self-update pin fixed to v0.8.30 in agent)
