...
- **0.8.29:** SA4000 doctor fix; dead code sweep; API status constants
 (0.8.17–0.8.29)
...
3. Reinstall drill with v0.8.29 binaries from Releases  

## Commands

```bash
brew reinstall netductor
netductor version   # 0.8.29
...
**Repo:** https://github.com/PavelNeyman/netductor  
**Release tag:** `v0.8.29` (check Releases if tag name differs)  
...
wget -qO /usr/local/bin/netductor \
  https://github.com/PavelNeyman/netductor/releases/download/v0.8.29/netductor-linux-amd64
...
## Current baseline — v0.8.29 (2026-09-20)

### Release
- Tag: **v0.8.29** · https://github.com/PavelNeyman/netductor/releases/tag/v0.8.29
- Assets: darwin/linux CLI, agent (amd64/arm/arm64/mipsle), tg, SHA256SUMS
- Homebrew Formula `0.8.29` · `brew reinstall netductor`
...
## 0.8.29
- Guest edge actions in `allowedEdgeActions`; TG waits `WaitCmdResult` (~90s) for status/grant
- TG **Certs** menu; OPEN_ITEMS: Path B dropped; plane ban documented
- Self-host git note: Forgejo/Gitea + CI + registry behind VPN

## Security review snapshot (v0.8.29)
...
4. **Hardcoded old download URLs** in some legacy docs/scripts (secondary self-update pin fixed to v0.8.29 in agent)
