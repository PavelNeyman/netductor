...
var formDict = map[string][2]string{
	...
	"net_configure_desc": {"Agent+UCI to final state; leave No if already set in LuCI", "Агент+UCI до конечного состояния; Нет = уже настроено в LuCI"},
	...
	"wifi_ssid":          {"Wi‑Fi SSID (2.4 + 5)", "Wi‑Fi SSID (2.4 + 5)"},
	"wifi_key":           {"Wi‑Fi password", "Пароль Wi‑Fi"},
	"wan_proto":          {"WAN mode (dhcp|static)", "WAN (dhcp|static)"},
